...
# 画像の取得
    ...
    # チェスボードのコーナーを検出
    ret, corner = cv2.findChessboardCorners(gray, pattern_size) # internal function which find a checkerboard and return corners coordinates
    # コーナーがあれば
    if ret == True:
        print("detected corner!")
        print(str(len(objpoints)+1) + "/" + str(reference_img))
        term = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_COUNT, 30, 0.1)
        cv2.cornerSubPix(gray, corner, (5,5), (-1,-1), term) # for high precision, do corner refinement
...
# objpoints: a vector consists of 3-d point vectors. 
# imgpoints: a vector consists of 2-d point vectors.
