    ...
    def get_mark_info(self, num_id):
        ...
        if num_id in np.ravel(ids) :
            ...
            for i, corner in enumerate(corners):
                ...
                elif -165 < int(euler_angle[0]) < -90: ## because default front value is wether -180~-170 or 170~180
                    #print("pitching down")
                    mc = 3
                ...
                elif (25 < int(euler_angle[1]) < 90) or (-90 < int(euler_angle[1]) < -25): ## or include yawing left, make the drag function to be used widely
                    #print("yawing right")
                    mc = 5
...
def extract_cdnt():
    ...
    while not exit_signal.is_set():
        ...
        #print(cam0_mark_search.get_mark_info(markID)) #属性（オブジェクトの変数）取得
        #print(flg)
        if (cam0_mark_search.get_mark_info(markID) == None):
            continue
        else:
            xi, yi, po = cam0_mark_search.get_mark_info(markID)
            if po == 0:
                flg = 1
            elif po == 5:
                flg = 2
            ...
            while flg == 3:
                try:
                    if po == 1 :
                        ...
                        drv.scroll(-1)
                    po = 0 ## to prevent endless function execution
...
if __name__ == "__main__":
    ### --- aruco設定 --- ###
    dict_aruco = aruco.Dictionary_get(aruco.DICT_4X4_50) # can this be deleted?
    parameters = aruco.DetectorParameters_create() # can this too?
    ...
    po = 0 ## pose info which is used between threads
    flg = 0 ## a flag which is used between threads
    main()
